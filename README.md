# Review Catalog Service

This microservice manages the review catalog and provides information about the available reviews.
...
- **Process management**: advanced reviewion process management using [PM2](https://pm2.keymetrics.io)
...
# run docker container in reviewion mode
yarn docker:prod
...
In reviewion mode, only `info`, `warn`, and `error` logs will be printed to the console.\
It is up to the server (or process manager) to actually read them from the console and store them in log files.\
This app uses pm2 in reviewion mode, which is already configured to store the logs in log files.
